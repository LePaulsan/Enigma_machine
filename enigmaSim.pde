...
int rotor0_init_Num = 24;
int rotor1_init_Num = 11;
int rotor2_init_Num = 19;

// put in rotor order name [rotorI, rotorII, rotorIII, rotorIV, rotorV]
char[][] first_slot = rotorII;
char[][] second_slot = rotorI;
char[][] third_slot = rotorIII;

//--------------------------------------------------------


Rotor rotor0 = new Rotor(rotor2_init_Num, third_slot);
Rotor rotor1 = new Rotor(rotor1_init_Num, second_slot);
Rotor rotor2 = new Rotor(rotor0_init_Num, first_slot);
...
void setup() {
  ...
  for (int i = 0; i < 26; i++) {
    if (i < 10) {
      lamps[i] = new Lamp(60*i + 50, 180, let[i]);
    } else if (i >= 10 && i < 19 ) {
      lamps[i] = new Lamp(60*(i - 10 ) + 90, 250, let[i]);
    } else if (i >= 19) {
      lamps[i] = new Lamp(60*(i - 19) + 130, 320, let[i]);
    }
  }
...
void keyPressed() {
  if (check) {
    ...
    //get the key, encrypt it, find the right lamp and lit it up
    char result = encrypt(key);
...
void keyReleased() {
  // put back the original back ground 
  draw_all_lamp();
  check = true;
}
